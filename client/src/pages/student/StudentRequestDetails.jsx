    ...
    <div className="min-h-screen bg-[#F5F7FA]">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-2xl border border-gray-200/80 shadow-md overflow-hidden animate-slide-up">
          ...
          {/* QR Code Segment for Approved Outpasses */}
          {outpass.status === "approved" && (
            <div className="px-6 py-8 border-b border-gray-100 bg-emerald-50/20 flex flex-col items-center justify-center">
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Scan the QR code below at the hostel exit gate.
                </p>
              </div>
              <div className="p-3 bg-white rounded-2xl shadow-md border border-emerald-100/50">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
                    `Outpass ID: ${outpass._id}\nName: ${outpass.student?.name || ""}\nRoll No: ${outpass.student?.enrollmentNo || ""}`
                  )}`}
                  alt="Gatepass QR"
                  className="w-36 h-36"
                />
              </div>
              <p className="text-xs font-mono text-gray-400 mt-3">ID: {outpass._id}</p>
            </div>
          )}

          {/* Back */}
          <div className="px-6 py-5 bg-gray-50/80 flex justify-between items-center">
            <button
              onClick={() => navigate(-1)}
            ...
            {outpass.status === "approved" && (
              <button
                ...
                className="btn btn-outline rounded-xl px-5 flex items-center gap-1.5 font-bold text-xs"
              >
